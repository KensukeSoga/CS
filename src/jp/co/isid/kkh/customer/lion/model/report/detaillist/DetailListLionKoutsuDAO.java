...
public class DetailListLionKoutsuDAO extends AbstractRdbDao implements  DetailListLionDaoInterface{
...
//	};
     ...
     * デフォルトコンストラクタ。
     */
    public DetailListLionKoutsuDAO() {
    ...
    private String getSelectSQLForItem() {
        ...
        sql.append("  " + TBTHB2KMEI.SONOTA2 + " " + DetailListLionVO.KEN_CD + ", ");
        sql.append("  mast1." + TBTHBAMST.FIELD2 + " " + DetailListLionVO.KEN_NAME + ", ");
        sql.append("  " + TBTHB2KMEI.SEIGAK + ", ");
        sql.append("  " + TBTHB2KMEI.NAME6 + " " + DetailListLionVO.ROSEN_NAME + ", ");
        sql.append("  " + TBTHB2KMEI.NAME7 + " " + DetailListLionVO.KIKAN + ", ");
    ...
    protected List<DetailListLionVO> createFindedModelInstances(List hashList) {
        ...
        if (getModel() instanceof DetailListLionVO) {
            for (int i = 0; i < hashList.size(); i++) {
                ...
                vo.setKenCd((String) result.get(DetailListLionVO.KEN_CD.toUpperCase().trim())); //県コード
                vo.setKenName((String) result.get(DetailListLionVO.KEN_NAME.toUpperCase().trim())); //県名
                vo.setSeiGak((BigDecimal) result.get(TBTHB2KMEI.SEIGAK.toUpperCase()));					//請求額
                vo.setRosenName((String) result.get(DetailListLionVO.ROSEN_NAME.toUpperCase()));					//路線名
                vo.setKikan((String) result.get(DetailListLionVO.KIKAN.toUpperCase()));					//期間
                vo.setName8((String) result.get(TBTHB2KMEI.NAME8.toUpperCase().trim()));		//その他
                //vo.setShohizeiGak((BigDecimal) result.get(TBTHB2KMEI.SEIGAK.toUpperCase()));		//消費税
