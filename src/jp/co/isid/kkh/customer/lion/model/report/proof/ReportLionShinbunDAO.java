...
public class ReportLionShinbunDAO extends AbstractRdbDao implements  ReportLionProofDaoInterface{
...
//		sql.append("  TRUNC(" + TBTHB2KMEI.SEIGAK + " * " + TBTHB2KMEI.RITU1 + " / 100 ) AS "+ ReportLionVO.SHOHIZEI_GAK +  ", "  );	//消費税
        sql.append("  TO_NUMBER(" + TBTHB2KMEI.NAME1 + ") AS " + ReportLionVO.SHOHIZEI_GAK + ", "); // 消費税
        sql.append("  mast5." + TBTHBAMST.SYBT		+ ", ");	//種別
        sql.append("  mast5." + TBTHBAMST.FIELD1		+ " AS " + ReportLionVO.BRAND_CD + ", ");	//ブランドコード
        sql.append("  mast5." + TBTHBAMST.FIELD2		+ " AS  " + ReportLionVO.BRAND_MEI + ", ");	//ブランド名
        ...
        sql.append("  mast11." + TBTHBAMST.FIELD1		+ " AS  " + ReportLionVO.SHINBUN_CD + ", ");	//新聞コード
        sql.append("  mast11." + TBTHBAMST.FIELD2		+ " AS  " + ReportLionVO.SHINBUN_MEI + ", ");	//新聞名
    ...
    protected List<ReportLionVO> createFindedModelInstances(List hashList) {
        ...
        if (getModel() instanceof ReportLionVO) {
            for (int i = 0; i < hashList.size(); i++) {
                ...
                vo.setShohizeiGak((BigDecimal) result.get(ReportLionVO.SHOHIZEI_GAK.toUpperCase().trim()));			//消費税
                vo.setShinCd((String) result.get(ReportLionVO.SHINBUN_CD.toUpperCase().trim()));   //新聞コード
                vo.setShinMei((String) result.get(ReportLionVO.SHINBUN_MEI.toUpperCase().trim())); //新聞名
