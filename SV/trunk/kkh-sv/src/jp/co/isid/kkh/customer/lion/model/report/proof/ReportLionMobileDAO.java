...
public class ReportLionMobileDAO extends AbstractRdbDao implements  ReportLionProofDaoInterface{
    ...
    protected List<ReportLionVO> createFindedModelInstances(List hashList) {
        ...
        if (getModel() instanceof ReportLionVO) {
            for (int i = 0; i < hashList.size(); i++) {
                ...
                vo.setKyokushiCd((String) result.get(ReportLionVO.KYOKUSHI_CD.toUpperCase().trim())); //局誌コード
                vo.setKyokushiName((String) result.get(ReportLionVO.KYOKUSHI_NAME.toUpperCase().trim())); //局誌名
