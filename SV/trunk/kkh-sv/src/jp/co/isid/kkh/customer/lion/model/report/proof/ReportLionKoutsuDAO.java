    ...
    protected List<ReportLionVO> createFindedModelInstances(List hashList) {
        ...
        if (getModel() instanceof ReportLionVO) {
            for (int i = 0; i < hashList.size(); i++) {
                ...
                vo.setKenCd((String) result.get(ReportLionVO.KEN_CD.toUpperCase().trim())); //県コード
                vo.setKenName((String) result.get(ReportLionVO.KEN_NAME.toUpperCase().trim())); //県名
                ...
                vo.setRosenName((String) result.get(ReportLionVO.ROSEN_NAME.toUpperCase()));					//路線名
                vo.setKikan((String) result.get(ReportLionVO.KIKAN.toUpperCase()));					//期間
